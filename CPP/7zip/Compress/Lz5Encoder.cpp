    ...
    case NCoderPropID::kLevel:
      {
        ...
        Byte zstd_level = static_cast < Byte > (LZ5MT_LEVEL_MAX);
  ...
  const UInt64 * /* outSize */ , ICompressProgressInfo *progress)
{
  ...
  LZ5MT_CCtx *ctx = LZ5MT_createCCtx(_numThreads, _props._level, _inputSize);
  if (!ctx)
      return S_FALSE;

  /* 3) compress */
  result = LZ5MT_CompressCCtx(ctx, &rdwr);
